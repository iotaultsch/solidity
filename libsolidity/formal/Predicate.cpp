...
{
	Predicate pred{_sort, _name, _context, _node};
	string functorName = pred.functor().name;
	solAssert(!m_predicates.count(functorName), "");
	m_predicates.emplace(functorName, move(pred));
	return &m_predicates.at(functorName);
}
...
Predicate::Predicate(
	...
	m_predicate(_sort, move(_name), _context),
	m_node(_node)
{
}
