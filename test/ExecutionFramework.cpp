	...
	m_vmPaths(_vmPaths)
{
	...
	setEvm(evmc_capabilities::EVMC_CAPABILITY_EVM1);
}

void ExecutionFramework::setEvm(evmc_capabilities _cap)
{
	if (_cap == evmc_capabilities::EVMC_CAPABILITY_EWASM)
	{
		solAssert(m_ewasmHost != nullptr, "");
		m_evmcHost = m_ewasmHost.get();
	}
	else if (_cap == evmc_capabilities::EVMC_CAPABILITY_EVM1)
	{
		solAssert(m_evmHost != nullptr, "");
		m_evmcHost = m_evmHost.get();
	}
	else
		solAssert(false, "Only EVMC_CAPABILITY_EVM1 or EVMC_CAPABILITY_EWASM supported.");
